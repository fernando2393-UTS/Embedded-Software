...
void* DataReadyUserArgumentsGlobal;      /*!< Private global pointer to the user arguments to use with the user callback function */
void (*DataReadyCallbackGlobal)(void *); /*!< Private global pointer to data ready user callback function */
...
bool Accel_Init(const TAccelSetup* const accelSetup)
{
  ...
  TI2CModule I2C = {ACCELEROMETER_ADDRESS,BAUD_RATE,accelSetup->readCompleteCallbackFunction,&I2C}; // I2C initialization

  DataReadyUserArgumentsGlobal = accelSetup->dataReadyCallbackArguments; // userArguments made globally(private) accessible
  DataReadyCallbackGlobal = accelSetup->dataReadyCallbackFunction; // userFunction made globally(private) accessible

  I2C_Init(&I2C,accelSetup->moduleClk); // Initialize I2C to 100000 baud rate
  I2C_SelectSlaveDevice(I2C.primarySlaveAddress); // Accelerometer address
...
void __attribute__ ((interrupt)) AccelDataReady_ISR(void)
{
  if (PORTB_PCR4 & PORT_PCR_IRQC_MASK)
  {
    if (PORTB_PCR4 & PORT_PCR_ISF_MASK) // Check if interrupt is pending
    {
      ...
      {
	      if(DataReadyCallbackGlobal)
	      {
	        (*DataReadyCallbackGlobal)(DataReadyUserArgumentsGlobal); // Accelerometer data ready callback function
	      }
      }
    }
  }
